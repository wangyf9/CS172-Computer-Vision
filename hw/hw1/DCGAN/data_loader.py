...
def get_data_loader(data_path, opts):
    ...
    if opts.data_aug == 'basic':
        ...
        #resize so that we can crop
        load_size = int(1.2 * opts.image_size)
        osize = [load_size, load_size]
        deluxe_transform = transforms.Compose([
            transforms.Resize(osize, Image.BICUBIC),
            transforms.RandomCrop(opts.image_size), ##crop size = ori
            transforms.RandomHorizontalFlip(),      ##horizontalfilp default prob = 0.5
            transforms.ToTensor(),                  ##To sensor
            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)), ##Three channels normalize \mu = 0.5 \sigma = 0.5
        ])
